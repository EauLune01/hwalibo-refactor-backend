...
public class ToiletController {
    ...
    @Operation(
            ...
            description = "특정 화장실의 ID를 이용해 화장실 상세 정보를 조회합니다."
    )
    @ApiResponses(value = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "화장실 상세 조회 성공"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "404", description = "해당 ID의 화장실을 찾을 수 없음")
    })
    @GetMapping("/{toiletId}")
    public ResponseEntity<ApiResponse<ToiletDetailResponse>> getToiletDetail(
